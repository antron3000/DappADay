...
const pIRAddress = "0xf22243140506f2a66107cd019f68a45c472df17b"
let pIR
...
async function initialize(web3) {
  ...
  await getApproved()
	...
	await populateAwardedTable()
	await populateReservedTable()
...
async function populateAwardedTable() {
	for(let j = 0;j<event_logs.awarded.length;j++){
		let alias = event_logs.awarded[j].alias
			let bountyName = event_logs.awarded[j].name
			let bountyDescription = event_logs.awarded[j].description
			let amount = event_logs.awarded[j].awardedAmount
			...
			amount = ethers.utils.commify(amount) + " " + symbol

			alias = document.createTextNode(alias)
			bountyName = document.createTextNode(bountyName)
			bountyDescription = document.createTextNode(bountyDescription)
			amount = document.createTextNode(amount)

			 cell1.appendChild(alias);
			 cell2.appendChild(bountyName);
			 cell3.appendChild(bountyDescription);
			 ...
			 awardedTable.appendChild(row);
	 }

}

async function populateReservedTable() {
	...
	for(let j = 0;j<event_logs.awarded.length;j++){
		...
		if(aliases.indexOf(alias)==-1){
			aliases.push(alias)
			console.log(alias)
			console.log("abc"==alias)
			let address = await pIR.aliasToHunter(alias)
