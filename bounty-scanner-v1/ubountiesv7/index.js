...
//testCash Faucet
let ubcAddress = "0x73aa31Cd548AC14713F778f454348d90564e2dE1"
let ubcABI = [
	{
		"inputs": [],
		"payable": false,
...
async function gatherEventLogs(){
	...
	let Topic = ethers.utils.id("rewarded(uint256,address,uint256,uint256)")
	console.log(Topic)
...
async function getRewardInfo(rewardLogs){
  ...
  for (n=0;n<rewardLogs.length;n++){
  		...
  		eventInfo.uBountyIndex = HexToInt(log[0],0)
			...
			let rewardAmount = HexToInt(log[2],decimals)
			let weiRewardAmount = HexToInt(log[3],18)
