...
async function initialize(){
  ...
  ubc = new ethers.Contract(ubcAddress,ubcABI,provider)
	await gatherAllEvents()
}

async function gatherAllEvents(){
	event_logs = new Array()
	let numSB = await ubc.numSingleBounties()
	let numMB = await ubc.numMultiBounties()

	for (let b=0; b<numSB;b++){
		let sbAddress = await ubc.singleBounties(b)
		console.log(sbAddress)
		let logs = await gatherEventLogs(sbAddress)
		event_logs.push(logs)
	}

	for (let c=0; c<numMB;c++){
		let mbAddress = await ubc.multiBounties(c)
		console.log(mbAddress)
		let logs = await gatherEventLogs(mbAddress)
		event_logs.push(logs)
	}
...
async function gatherEventLogs(bAddress){
	console.log("gather Logs")

	let Topic = ethers.utils.id("Reward(address,uint256)")
	console.log(Topic)
	let Filter = createFilter(Topic,bAddress)
	console.log(Filter)
...
function createFilter(topic,bAddress){
  let filter = {
		address: bAddress,
		fromBlock: fromBlock,
		toBlock: toBlock,
...
async function getRewardInfo(rewardLogs){
  ...
  for (n=0;n<rewardLogs.length;n++){
  		...
  		eventInfo.bountyHunter = HexToAddress(log[0])
			eventInfo.reward = HexToInt(log[1],decimals)
