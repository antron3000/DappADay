...
//testCash Faucet
let ubcAddress = "0x7DE09eE61Fd4c326098bE7C4C86b80408707DB9b"
let ubcABI = [
	{
		"inputs": [],
		"payable": false,
...
async function gatherEventLogs(){
	...
	let Topic = "0xafb331faabf2bd7665787e4369a555475a58ec22ba040160f7cb19b02176b33b"
	console.log(Topic)
...
async function getRewardInfo(rewardLogs){
  ...
  for (n=0;n<rewardLogs.length;n++){
		console.log(rewardLogs[n])
  		let log = rewardHexArray[n]
			console.log(log)
  		let eventInfo = new Object()

  		eventInfo.uBountyIndex = HexToInt(log[0],4)
			eventInfo.hunter = HexToAddress(log[1])
			let rewardAmount = HexToInt(log[3],decimals)
...
async function populateRewards(){
	...
	for(let n = 0;n<event_logs.length;n++){
		...
		let bountyName = "name"
		let bountyHunter = event_logs[n].hunter
